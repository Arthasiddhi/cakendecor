...
/**
 * Test baker ajax login controller
 */
namespace CND\Baker\Test\Unit\Controller\Ajax;
...
{
    /**
     * @var \CND\Baker\Controller\Ajax\Login
     */
    protected $object;
    ...
    protected function setUp()
    {
        ...
        $this->bakerSession = $this->createPartialMock(\CND\Baker\Model\Session::class, [
                'isLoggedIn',
                'getLastBakerId',
                'getBeforeAuthUrl',
                'setBeforeAuthUrl',
                'setBakerDataAsLoggedIn',
                'regenerateId'
            ]);
        ...
        $this->bakerAccountManagementMock =
            $this->createPartialMock(\CND\Baker\Model\AccountManagement::class, ['authenticate']);
        ...
        $this->object = $objectManager->getObject(
            \CND\Baker\Controller\Ajax\Login::class,
            [
                'context' => $contextMock,
    ...
    {
        $jsonRequest = '{"username":"baker@example.com", "password":"password"}';
        ...
        $this->jsonHelperMock
            ...
            ->method('jsonDecode')
            ->with($jsonRequest)
            ->willReturn(['username' => 'baker@example.com', 'password' => 'password']);

        $bakerMock = $this->getMockForAbstractClass(\CND\Baker\Api\Data\BakerInterface::class);
        $this->bakerAccountManagementMock
            ->expects($this->any())
            ->method('authenticate')
            ->with('baker@example.com', 'password')
        ...
        $this->bakerSession->expects($this->once())->method('regenerateId');

        $redirectMock = $this->createMock(\CND\Baker\Model\Account\Redirect::class);
        ...
        $scopeConfigMock->expects($this->once())->method('getValue')
            ->with('baker/startup/redirect_dashboard')
            ->willReturn(0);
    ...
    public function testLoginFailure()
    {
        ...
        $this->bakerSession->expects($this->never())
            ->method('setBakerDataAsLoggedIn')
            ->with($bakerMock);

        $this->bakerSession->expects($this->never())->method('regenerateId');
