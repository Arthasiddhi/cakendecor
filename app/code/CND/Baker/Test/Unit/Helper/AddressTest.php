...
class AddressTest extends \PHPUnit\Framework\TestCase
{
    ...
    /** @var \CND\Baker\Model\Address\Config|\PHPUnit_Framework_MockObject_MockObject */
    ...
    protected function setUp()
    {
        ...
        $className = \CND\Baker\Helper\Address::class;
        ...
        $this->bakerMetadataService = $arguments['bakerMetadataService'];
    ...
    {
        $this->helper = new \CND\Baker\Helper\Address(
            $this->context,
            $blockFactory,
            $this->storeManager,
    ...
    public function testGetConfigCanShowConfig()
    {
        ...
        $this->scopeConfig->expects($this->once())//test method cache
            ->method('getValue')
            ->with('baker/address', \Magento\Store\Model\ScopeInterface::SCOPE_STORE, $store)
    ...
    public function testIsVatValidationEnabled($store, $result)
    {
        $this->scopeConfig->expects($this->once())
            ->method('getValue')
            ->with(
                \CND\Baker\Helper\Address::XML_PATH_VAT_VALIDATION_ENABLED,
    ...
    public function testHasValidateOnEachTransaction($store, $result)
    {
        $this->scopeConfig->expects($this->once())
            ->method('getValue')
            ->with(
                \CND\Baker\Helper\Address::XML_PATH_VIV_ON_EACH_TRANSACTION,
                \Magento\Store\Model\ScopeInterface::SCOPE_STORE,
                $store
            )
            ->will($this->returnValue($result));
    ...
    public function testGetTaxCalculationAddressType($store, $result)
    {
        $this->scopeConfig->expects($this->once())
            ->method('getValue')
            ->with(
                \CND\Baker\Helper\Address::XML_PATH_VIV_TAX_CALCULATION_ADDRESS_TYPE,
                \Magento\Store\Model\ScopeInterface::SCOPE_STORE,
                $store
            )
            ->will($this->returnValue($result));
    ...
    public function testIsDisableAutoGroupAssignDefaultValue()
    {
        $this->scopeConfig->expects($this->once())
            ->method('getValue')
            ->with(
                \CND\Baker\Helper\Address::XML_PATH_VIV_DISABLE_AUTO_ASSIGN_DEFAULT,
                \Magento\Store\Model\ScopeInterface::SCOPE_STORE
            )
    ...
    public function testIsVatAttributeVisible()
    {
        $this->scopeConfig->expects($this->once())
            ->method('getValue')
            ->with(
                \CND\Baker\Helper\Address::XML_PATH_VAT_FRONTEND_VISIBILITY,
                \Magento\Store\Model\ScopeInterface::SCOPE_STORE
            )
            ->will($this->returnValue(true));
    ...
    /**
     * @param string $code
     * @param \CND\Baker\Block\Address\Renderer\RendererInterface|null $result
    ...
    {
        $renderer = $this->getMockBuilder(\CND\Baker\Block\Address\Renderer\RendererInterface::class)
            ->disableOriginalConstructor()->getMock();
        return [
            ['valid_code', $renderer],
            ['invalid_code', null],
    ...
    public function testIsAttributeVisible($attributeCode, $isMetadataExists)
    {
        ...
        if ($isMetadataExists) {
            $attributeMetadata = $this->getMockBuilder(\CND\Baker\Api\Data\AttributeMetadataInterface::class)
                ->getMockForAbstractClass();
