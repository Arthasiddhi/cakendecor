...
use CND\Baker\Observer\BeforeAddressSaveObserver;
...
{
    /**
     * @var \CND\Baker\Observer\BeforeAddressSaveObserver
     */
    protected $model;
    ...
    protected function setUp()
    {
        ...
        $this->helperAddress = $this->getMockBuilder(\CND\Baker\Helper\Address::class)
            ->disableOriginalConstructor()
            ->getMock();
    ...
    public function testBeforeAddressSaveWithBakerAddressId()
    {
        $bakerAddressId = 1;
        ...
        $this->registry->expects($this->once())
            ->method('register')
            ->with(BeforeAddressSaveObserver::VIV_CURRENTLY_SAVED_ADDRESS, $bakerAddressId)
            ->willReturnSelf();
     ...
     * @dataProvider dataProviderBeforeAddressSaveWithoutBakerAddressId
     */
    public function testBeforeAddressSaveWithoutBakerAddressId(
        $configAddressType,
        $isDefaultBilling,
        $isDefaultShipping
    ) {
        ...
        $this->registry->expects($this->any())
            ->method('register')
            ->willReturnMap([
                [BeforeAddressSaveObserver::VIV_CURRENTLY_SAVED_ADDRESS, $bakerAddressId, false, $this->registry],
     ...
     * @return array
     */
    public function dataProviderBeforeAddressSaveWithoutBakerAddressId()
    {
        return [
            [
