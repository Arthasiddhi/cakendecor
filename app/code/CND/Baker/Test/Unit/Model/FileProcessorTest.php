...
class FileProcessorTest extends \PHPUnit\Framework\TestCase
{
    ...
    public function testGetStat()
    {
        ...
        $this->mediaDirectory->expects($this->once())
            ->method('stat')
            ->with(BakerMetadataInterface::ENTITY_TYPE_CUSTOMER . $fileName)
            ->willReturn(['size' => 1]);

        $model = $this->getModel(BakerMetadataInterface::ENTITY_TYPE_CUSTOMER);
    ...
    public function testGetViewUrlBaker()
    {
        ...
        $this->urlBuilder->expects($this->once())
            ->method('getUrl')
            ->with('baker/index/viewfile', ['image' => $encodedFilePath])
            ->willReturn($fileUrl);
    ...
    public function testGetViewUrlBakerAddress()
    {
        $filePath = 'filename.ext1';
    ...
    public function testSaveTemporaryFile()
    {
        ...
        $this->uploaderFactory->expects($this->once())
            ->method('create')
            ->with(['fileId' => 'baker[' . $attributeCode . ']'])
            ->willReturn($uploaderMock);

        $this->mediaDirectory->expects($this->once())
            ->method('getAbsolutePath')
            ->with(BakerMetadataInterface::ENTITY_TYPE_CUSTOMER . '/' . FileProcessor::TMP_DIR)
            ->willReturn($absolutePath);

        $model = $this->getModel(BakerMetadataInterface::ENTITY_TYPE_CUSTOMER, $allowedExtensions);
        $result = $model->saveTemporaryFile('baker[' . $attributeCode . ']');
    ...
    public function testSaveTemporaryFileWithError()
    {
        ...
        $model = $this->getModel(BakerMetadataInterface::ENTITY_TYPE_CUSTOMER, $allowedExtensions);
        $model->saveTemporaryFile('baker[' . $attributeCode . ']');
    }

    /**
     * @expectedException \Magento\Framework\Exception\LocalizedException
     * @expectedExceptionMessage Unable to create directory baker/f/i
     */
    public function testMoveTemporaryFileUnableToCreateDirectory()
    {
        $filePath = '/filename.ext1';

        $destinationPath = 'baker/f/i';
    ...
    public function testMoveTemporaryFile()
    {
        ...
        $path = BakerMetadataInterface::ENTITY_TYPE_CUSTOMER . '/' . FileProcessor::TMP_DIR . $filePath;
    ...
    {
        $fileName = '/filename.ext1';
        $absoluteFilePath = '/absolute_path/baker/filename.ext1';

        $expected = 'ext1';

        $this->mediaDirectory->expects($this->once())
            ->method('getAbsolutePath')
            ->with(BakerMetadataInterface::ENTITY_TYPE_CUSTOMER . '/' . ltrim($fileName, '/'))
