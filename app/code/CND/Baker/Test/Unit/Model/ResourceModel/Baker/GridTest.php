 ...
 */
namespace CND\Baker\Test\Unit\Model\ResourceModel\Baker;

use CND\Baker\Model\ResourceModel\Baker\Grid;
...
class GridTest extends \PHPUnit\Framework\TestCase
{
    ...
    public function testSyncBakerGrid()
    {
        $gridTable = 'baker_grid_flat';
        $bakerLogTable = 'baker_log';

        $this->indexerRegistry->expects($this->once())
            ->method('get')
            ->with(\CND\Baker\Model\Baker::CUSTOMER_GRID_INDEXER_ID)
        ...
        $this->queryResult->expects($this->once())
            ->method('fetchAll')
            ->willReturn([['baker_id' => 23], ['baker_id' => 65]]);
        $this->indexer->expects($this->once())
            ->method('reindexList')
            ->with(['23', '65']);

        $this->observer->syncBakerGrid();
    }
}
