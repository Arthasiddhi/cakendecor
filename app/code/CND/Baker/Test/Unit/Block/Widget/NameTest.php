...
use CND\Baker\Block\Widget\Name;

/**
 * Test class for \CND\Baker\Block\Widget\Name.
 *
...
class NameTest extends \PHPUnit\Framework\TestCase
{
    ...
    const DEFAULT_CLASS_NAME = 'baker-name';
    ...
    /** @var  \PHPUnit_Framework_MockObject_MockObject | \CND\Baker\Model\Options */
    private $_options;
    ...
    /** @var \PHPUnit_Framework_MockObject_MockObject|\CND\Baker\Api\AddressMetadataInterface */
    ...
    protected function setUp()
    {
        ...
        $addressHelper = $this->createMock(\CND\Baker\Helper\Address::class);

        $this->_options = $this->createMock(\CND\Baker\Model\Options::class);
        ...
        $this->addressMetadata = $this->getMockBuilder(\CND\Baker\Api\AddressMetadataInterface::class)
            ->getMockForAbstractClass();
        $this->addressMetadata->expects($this->any())
            ->method('getAttributeMetadata')
            ->will($this->returnValue($this->attribute));

        $this->_block = new \CND\Baker\Block\Widget\Name(
            $context,
            $addressHelper,
            $this->bakerMetadata,
            $this->_options,
    ...
    {
        $this->_setUpShowAttribute([\CND\Baker\Model\Data\Baker::PREFIX => self::PREFIX]);
    ...
    {
        $this->_setUpShowAttribute([\CND\Baker\Model\Data\Baker::MIDDLENAME => self::MIDDLENAME]);
    ...
    {
        $this->_setUpShowAttribute([\CND\Baker\Model\Data\Baker::SUFFIX => self::SUFFIX]);
    ...
    {
        /**
         * Added some padding so that the trim() call on Baker::getPrefix() will remove it. Also added
         * special characters so that the escapeHtml() method returns a htmlspecialchars translated value.
         */
        $baker = $this->getMockBuilder(
            \CND\Baker\Api\Data\BakerInterface::class)->getMockForAbstractClass(
            );
        $baker->expects($this->once())->method('getPrefix')->willReturn('  <' . self::PREFIX . '>  ');

        $this->_block->setObject($baker);
    ...
    {
        /**
         * Added padding and special characters to show that trim() works on Baker::getSuffix() and that
         * a properly htmlspecialchars translated value is returned.
         */
        $baker = $this->getMockBuilder(
            \CND\Baker\Api\Data\BakerInterface::class)->getMockForAbstractClass(
            );
        $baker->expects($this->once())->method('getSuffix')->willReturn('  <' . self::SUFFIX . '>  ');
        $this->_block->setObject($baker);
     ...
     *
     * @param array $data Baker attribute(s)
    ...
    private function _setUpShowAttribute(array $data)
    {
         ...
         */
        $this->_block->setForceUseBakerAttributes(true);
        $this->_block->setObject($baker);
    ...
    private function _setUpIsAttributeRequired()
    {
         ...
         */
        $this->_block->setForceUseBakerAttributes(false);
        $this->_block->setForceUseBakerRequiredAttributes(true);
