 ...
 */
namespace CND\Baker\Block\Adminhtml\Sales\Order\Address\Form\Renderer;
...
class Vat extends \Magento\Backend\Block\Widget\Form\Renderer\Fieldset\Element
{
    ...
    public function getValidateButton()
    {
        if ($this->_validateButton === null) {
            ...
            $validateUrl = $this->_urlBuilder->getUrl('baker/system_config_validatevat/validateAdvanced');

            $groupMessage = __(
                'The baker is now assigned to Baker Group %s.'
            ) . ' ' . __(
                'Would you like to change the Baker Group for this order?'
            );
            ...
            $vatValidateOptions = $this->_jsonEncoder->encode(
                [
                    ...
                    'vatValidAndGroupValidMessage' => __(
                        'The VAT ID is valid. The current Baker Group will be used.'
                    ),
                    'vatValidAndGroupInvalidMessage' => __(
                        'The VAT ID is valid but no Baker Group is assigned for it.'
                    ),
                    'vatValidAndGroupChangeMessage' => __(
                        'Based on the VAT ID, the baker belongs to the Baker Group %s.'
                    ) . "\n" . $groupMessage,
                    'vatValidationFailedMessage' => __(
                        'Something went wrong while validating the VAT ID.'
                    ),
                    'vatBakerGroupMessage' => __(
                        'The baker would belong to Baker Group %s.'
                    ),
                    'vatGroupErrorMessage' => __('There was an error detecting Baker Group.'),
                ]
            );
