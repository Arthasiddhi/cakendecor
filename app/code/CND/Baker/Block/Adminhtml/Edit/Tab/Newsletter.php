...
class Newsletter extends \Magento\Backend\Block\Widget\Form\Generic implements TabInterface
{
    ...
    public function initForm()
    {
        ...
        $subscriber = $this->_subscriberFactory->create()->loadByBakerId($bakerId);
        ...
        if ($this->bakerAccountManagement->isReadonly($bakerId)) {
        ...
        $this->updateFromSession($form, $bakerId);
     ...
     * @return void
     */
    protected function updateFromSession(\Magento\Framework\Data\Form $form, $bakerId)
    {
        $data = $this->_backendSession->getBakerFormData();
        if (!empty($data)) {
            $dataBakerId = isset($data['baker']['entity_id']) ? $data['baker']['entity_id'] : null;
            if (isset($data['subscription']) && $dataBakerId == $bakerId) {
