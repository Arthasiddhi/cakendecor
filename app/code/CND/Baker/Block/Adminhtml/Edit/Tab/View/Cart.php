...
/**
 * Adminhtml baker cart items grid block
 *
...
class Cart extends \Magento\Backend\Block\Widget\Grid\Extended
{
    ...
    {
        parent::_construct();
        $this->setId('baker_view_cart_grid');
        $this->setDefaultSort('added_at', 'desc');
        $this->setSortable(false);
        $this->setPagerVisibility(false);
        $this->setFilterVisibility(false);
        $this->setEmptyText(__('There are no items in baker\'s shopping cart.'));
    }
    ...
    protected function getQuote()
    {
        if (null == $this->quote) {
            ...
            $currentBakerId = $this->_coreRegistry->registry(RegistryConstants::CURRENT_CUSTOMER_ID);
            if (!empty($currentBakerId)) {
                try {
                    $this->quote = $this->quoteRepository->getForBaker($currentBakerId, $storeIds);
