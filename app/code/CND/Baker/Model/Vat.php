...
{
    /**
     * Config paths to VAT related baker groups
     */
    const XML_PATH_CUSTOMER_VIV_INTRA_UNION_GROUP = 'baker/create_account/viv_intra_union_group';

    const XML_PATH_CUSTOMER_VIV_DOMESTIC_GROUP = 'baker/create_account/viv_domestic_group';

    const XML_PATH_CUSTOMER_VIV_INVALID_GROUP = 'baker/create_account/viv_invalid_group';

    const XML_PATH_CUSTOMER_VIV_ERROR_GROUP = 'baker/create_account/viv_error_group';
     ...
     */
    const XML_PATH_CUSTOMER_GROUP_AUTO_ASSIGN = 'baker/create_account/auto_group_assign';
    ...
    /**
     * Retrieve baker group ID based on his VAT number
     *
     * @param string $bakerCountryCode
     * @param DataObject $vatValidationResult
     * @param \Magento\Store\Model\Store|string|int $store
     * @return null|int
     */
    public function getBakerGroupIdBasedOnVatNumber($bakerCountryCode, $vatValidationResult, $store = null)
    {
        $groupId = null;
        ...
        $vatClass = $this->getBakerVatClass($bakerCountryCode, $vatValidationResult, $store);
     ...
     * @return null|string
     */
    public function getBakerVatClass($bakerCountryCode, $vatValidationResult, $store = null)
    {
        $vatClass = null;

        $isVatNumberValid = $vatValidationResult->getIsValid();

        if (is_string($bakerCountryCode)
            && !empty($bakerCountryCode)
            && $bakerCountryCode === $this->getMerchantCountryCode($store)
            && $isVatNumberValid
        ) {
