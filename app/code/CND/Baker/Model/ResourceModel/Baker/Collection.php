...
class Collection extends \Magento\Eav\Model\Entity\Collection\VersionControl\AbstractCollection
{
     ...
     */
    const CUSTOMER_MODEL_NAME = \CND\Baker\Model\Baker::class;
    ...
    {
        $this->_init($this->_modelName, \CND\Baker\Model\ResourceModel\Baker::class);
    }

    /**
     * Group result by baker email
     *
     * @return $this
     */
    ...
    {
        $fields = [];
        $bakerAccount = $this->_fieldsetConfig->getFieldset('baker_account');
        foreach ($bakerAccount as $code => $field) {
            if (isset($field['name'])) {
                $fields[$code] = $code;
            }
        }
